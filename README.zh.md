...
- 完成时，`/mino-checkup done` 会发**一条汇总 comment**，内联所有 event yml，以便本地日志丢失时仍可从 GitHub 单独 replay 工作流。

v1.10 之前的问题（per-event comment）继续可被 `/mino-checkup reconcile` 作为 fallback 来源读取。
