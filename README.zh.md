...
> v0.6.3 — **回复评论 + 可选 Note 入参 + 状态评论默认静默。** GitHub issue 评论拆分为两类：**状态评论**（轻量，仅用于打断）和 **回复评论**（会话式，承载内容）。`checkup_done` 与 `verify_passed` 不再发状态评论。新增 `<note>` 入参，让人工验证者把散文喂给 agent 用于回复决策。协议加法变更（v1.13 保持）：§ Comment Classes、§ Reply Dispatch、可选事件字段 `reply_posted`。
> v0.6.2 — **轻量评论清理 + Commit 自动链接。** 偿还 v0.5.2 债务：GitHub  audible 评论现在仅从 `comment-*.md.tmpl` 文件渲染（无 yaml fence，无 `.mino/*` 路径）。`Commit:` 自动链接已接入所有 audible 评论。协议新增 § Slim Comment Invariant（加法变更；v1.13 保持不变）。
